...
//This version of the list contains move and copy constructors
//These are needed for the repopulate deck function
...
List<T>::List(const List<T> &src) :
	m_container(new T[src.m_max_capacity]),
	m_max_capacity(src.m_max_capacity),
	m_size(0)
{
	for (int i = 1; i <= src.getLength(); ++i)
		insert(m_size + 1, src.retrieve(i));
}
...
List<T> &List<T>::operator=(const List<T> &src)
{
	...
	// cout << "OL copy assignment called" << endl;
	return *this;
}
...
List<T>::List(List<T> &&other) :
	...
	m_size(0)
{
	std::swap(other.m_max_capacity, m_max_capacity);
...
List<T> &List<T>::operator=(List<T> &&other)
{
	...
	// cout << "OL move assignment called" << endl;
	return *this;
}
